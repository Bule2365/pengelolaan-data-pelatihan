        ...
        <form action="{{ route('events.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="event_description" id="event_description" rows="4"
                    class="form-textarea w-full border-gray-300 rounded-md shadow-sm">{{ old('event_description', $event->event_description) }}</textarea>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="price" id="price"
                    class="form-input w-full border-gray-300 rounded-md shadow-sm" value="{{ old('price', $event->price) }}"
                    required>
                @error('price')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            ...
            <div>
                ...
                <select name="hotel_id" id="hotel_id" class="form-select w-full border-gray-300 rounded-md shadow-sm"
                    {{ old('event_type', $event->event_type) == 'online' ? 'disabled' : '' }}>
                    <option value="">-- Choose a Hotel --</option>
                    @foreach ($hotels as $hotel)
                        <option value="{{ $hotel->id }}"
                            {{ old('hotel_id', $event->hotel_id) == $hotel->id ? 'selected' : '' }}>
                            {{ $hotel->name }}
                        </option>
                    @endforeach
                </select>
                ...
                <select name="trainer_id" id="trainer_id" class="form-select w-full border-gray-300 rounded-md shadow-sm"
                    ...
                    @foreach ($trainers as $trainer)
                        <option value="{{ $trainer->id }}"
                            {{ old('trainer_id', $event->trainer_id) == $trainer->id ? 'selected' : '' }}>
                            {{ $trainer->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="event_type" id="event_type" class="form-select w-full border-gray-300 rounded-md shadow-sm"
                    ...
                    <option value="online" {{ old('event_type', $event->event_type) == 'online' ? 'selected' : '' }}>Online
                    </option>
                    <option value="offline" {{ old('event_type', $event->event_type) == 'offline' ? 'selected' : '' }}>
                        Offline</option>
                </select>
                @error('event_type')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
            ...
            <div>
                ...
                <input type="file" name="image" id="image"
                    class="form-input w-full border-gray-300 rounded-md shadow-sm">
                @if ($event->image)
                    <img src="{{ asset('storage/' . $event->image) }}" alt="Event Image" class="w-24 mt-2">
                @endif
                @error('image')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
