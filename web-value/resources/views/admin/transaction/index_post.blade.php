...
@section('title', 'Kelola Order Posts')

@section('header', 'Transaksi Post')
            ...
            <tbody>
                @foreach ($orders as $order)
                    <tr>
                        ...
                        <td class="border px-4 py-2">{{ $order->training->post->dataPrice->training_title }}</td>
                        <td class="border px-4 py-2">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</td>
                        <!-- Menampilkan nama metode pembayaran langsung -->
                        <td class="border px-4 py-2">
                            {{ $order->paymentMethod->method_name ?? 'Belum dipilih' }}
                        </td>
                        <td class="border px-4 py-2">
                            <form action="{{ route('admin.order.post.updateStatus', $order->id) }}" method="POST">
                                @csrf
                                @method('PUT')
